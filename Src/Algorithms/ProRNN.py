...
class ProRNN(Agent):
    def __init__(self, config):
        super(ProRNN, self).__init__(config)
    ...
    def optimize(self):
        ...
        for iter in range(self.config.max_inner):
            ...
            ####################################################################################################
            ### del_extrapolator: equation (5)-(a) ## this is the one shoul be replaced                                                           ###
            ...
            if self.config.entropy_lambda > 0:
                if self.config.cont_actions:
                    entropy = torch.sum(dist_all.entropy().view(B, H, -1).sum(dim=-1) * mask) / torch.sum(
                        mask)  # (BxH)xA -> BxH
                else:
